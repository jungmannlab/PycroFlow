...
class TestOrchestration(unittest.TestCase):
    ...
    def test_01(self):
        ...
        fh = por.FluidHandler(
            MagicMock(), _wrap(protocol_fluid), threadexchange
        )
    ...
    def test_02(self):
        ...
        fh = por.FluidHandler(
            dummy_system, _wrap(protocol_fluid), threadexchange
        )
        ...
        # call list.
        self.assertIn(
            call.execute_protocol_entry(0), dummy_system.method_calls
        )
    ...
    def test_03(self):
        ...
        fh = por.ImagingHandler(
            dummy_system, _wrap(protocol_img), threadexchange
        )
        threadexchange["start_protocol_flag"].set()
        fh.start()
        # now running in separate thread
    ...
    def test_04(self):
        ...
        dummy_system = MagicMock()
        fh = por.IlluminationHandler(
            dummy_system, _wrap(protocol_illu), threadexchange
        )
        threadexchange["start_protocol_flag"].set()
        fh.start()
        # now running in separate thread
    ...
    def test_get_step_progress_handler_and_system(self):
        ...
        fh = por.ImagingHandler(
            system,
            _wrap([{"$type": "acquire", "frames": 10, "t_exp": 1}]),
            threadexchange,
        )
    ...
    def test_incubate_sets_step_progress(self):
        ...
        fh = por.FluidHandler(
            MagicMock(),
            _wrap([{"$type": "incubate", "duration": 0.3}]),
            threadexchange,
        )
        seen = []
    ...
    def test_illumination_handler_assigns_protocol(self):
        ...
        protocol = {
            "illu": _wrap([{"$type": "set power", "laser": 1, "power": 5}])
        }

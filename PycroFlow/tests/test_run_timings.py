...
class TestRunRecord(unittest.TestCase):
    ...
    def _service(self, tmp):
        ...
        svc._protocol = {
            "fluid": {"protocol_entries": [{"$type": "inject", "volume": 100}]}
        }
        return svc
    ...
    def test_saves_design_and_run_sequence(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(any(p.endswith("_design.yaml") for p in written))
            self.assertTrue(
    ...
    def test_second_run_does_not_overwrite_the_first(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch(
                "PycroFlow.services.experiment_service.datetime"
            ) as dt:
...
class TestReservoirPreflight(unittest.TestCase):
    ...
    def test_system_without_reservoirs_is_not_second_guessed(self):
        ...
        svc._protocol = {
            "fluid": {
                "protocol_entries": [{"$type": "inject", "reservoir_id": 9}]
            }
        }
        self.assertEqual(svc.check_reservoirs_routable(), [])

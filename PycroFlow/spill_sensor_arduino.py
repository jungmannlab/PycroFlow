...
class ArduinoSensorInterface:
        ...
        while not self.monitor_abort_flag.is_set():
            ...
            try:
                ...
                if "WET" in response:
                    ...
                    return
                elif "DRY" in response:
                    pass
                else:
                    logger.debug(f"Unexpected poll response: '{response}'")
                    self.serial_conn.flush()
                    response = self.serial_conn.readline().decode().strip()
                    logger.debug(f"after flushing: {response}")
                    # return None
            except Exception as e:
    ...
    def start_broadcast(self):
        if not self.is_connected:
            ...
            return None
        try:
            self.serial_conn.write(self.START_BROADCAST_CMD)
            self.serial_conn.flush()
            response = self.serial_conn.readline().decode().strip()
            if response == "START_BROADCAST_OK":
                print("✓ Starting read value broadcast")
                return True
            else:
                print(f"Unexpected poll response: '{response}'")
                return None
    ...
    def stop_broadcast(self):
        if not self.is_connected:
            ...
            return None
        try:
            self.serial_conn.write(self.STOP_BROADCAST_CMD)
            self.serial_conn.flush()
            response = self.serial_conn.readline().decode().strip()
            if response == "STOP_BROADCAST_OK":
                print("✓ Stopping read value broadcast")
                return True
            else:
                print(f"Unexpected poll response: '{response}'")
                return None

...
def fmt_time_delta(time_delta, width=20):
    """Format the time delta in a readable string.
    ...
    width : int, default: 20
            Width to pad or truncate the result to.
    ...
    snippets = [
        "{:d} {:s}".format(n, u) for n, u in zip(numbers, units) if n > 0
    ]
...
class ProgressBar:
    ...
    def progress(self, x):
        ...
        print(
            ...
            + "]"
            + "  {:d}/{:d}".format(
                1 + int(x * self.nimgs_total), self.nimgs_total
            )
            + " time remaining: {:s}".format(
                fmt_time_delta(time_left, self.timewidth)
            ),
            end="\r",
        )
    ...
    def progress_increment(self):
        """Increment ``nimgs_acquired`` by 1 and refresh the progress bar."""

"""PyQt6 GUI frontend for PycroFlow.

Importing this package does NOT import PyQt6 — the Qt-dependent modules
(``app``, ``main_window``, ``qt_bridge``, ``tabs.*``) import it themselves
and are only loaded when the GUI is actually launched. This keeps
``import PycroFlow.gui`` safe in environments without PyQt6 (CI, headless

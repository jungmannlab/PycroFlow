...
def optional_break(timeout=5):
    try:
        ipt = inputimeout(
            "Proceed? [Y/N - default Y, respond within {:.1f}s]".format(
                timeout
            ),
            timeout=timeout,
        )
...
def main(acquisition_config, dry_run=False, break_for_slide=True):
    ...
    if "illu_parameters" in acquisition_config.keys():
        ...
        laserlaunch.laser = acquisition_config["illu_parameters"][
            "laser"
        ]  # nm
    ...
    events = multi_d_acquisition_events(
        num_time_points=10, time_interval_s=0.1
    )
    with Acquisition(
        directory=acquisition_config["save_dir"],
    ...
    if not dry_run:
        ...
        if acquisition_config["aria_parameters"]["use_TTL"]:
            ...
            print(
                "Please set Aria TTL duration to 300 ms, load Aria protocol {:s} and start it now.".format(
                    protocol_file
                )
            )
        else:
            aria = AriaConnection()
            print(
                "Please load Aria protocol {:s} and start it now.".format(
                    protocol_file
                )
            )
    ...
    for round, desc in enumerate(imground_descriptions):
        ...
        if not dry_run:
            print(
                "waiting for Aria pulsing to signal readiness for round {:d}".format(
                    round
                )
            )
            aria.sense_trigger()
        ...
        if round == 0 and break_for_slide:
            ...
            input(
                "Check focus. Stop Live View when done. Press Enter to continue."
            )
        ...
        if "illu_parameters" in acquisition_config.keys():
            laserlaunch.power = acquisition_config["illu_parameters"][
                "power"
            ]  # mW

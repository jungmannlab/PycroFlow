    ...
    for i in range(pfs_range.shape[1]):
        ...
        for j, pfs in enumerate(pfs_range[:, i]):
            ...
            if wait_for_focus(core, tag_status, prop_status):
                ...
                zpos[j, i] = pos
        plot_calibration(
            pfs_range, zpos, show=False, range_pars=range_pars, sleep=sleep
        )
        np.save(
            "pfs_range_{:d}_{:d}_{:d}_sleep{:d}.npy".format(
                range_pars[0],
                range_pars[1],
                int(range_pars[2] * 1000),
                int(sleep * 1000),
            ),
            pfs_range,
        )
        np.save(
            "zpos_{:d}_{:d}_{:d}.npy".format(
                range_pars[0],
                range_pars[1],
                int(range_pars[2] * 1000),
                int(sleep * 1000),
...
def plot_calibration(
    pfs_range, zpos, show=True, range_pars=(150, 400, 0.005), sleep=0
):
    ...
    fig.savefig(
        "calibration_{:d}_{:d}_{:d}_sleep{:d}.png".format(
            range_pars[0],
            range_pars[1],
            int(range_pars[2] * 1000),
            int(sleep * 1000),
...
def wait_for_focus(core, tag_status, prop_status, timeout=1):
    ...
    while time.time() - tic < timeout:
        ...
        if status == "Locked in focus":
            ...
            status == "In Range"
        ):  # check whether 'PFS in Range' is actually the correct property to use on Skylab
            time.sleep(0.02)
            return True

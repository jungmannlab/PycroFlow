...
def zpos2pfsoffset(zpos, curr_offset, calibration=None):
    ...
    if calibration is not None:
        ...
        pfs_values = np.interp(zpos, z_relative, calibration[:, 1])
        return pfs_values
...
def move_pfsoffset(
    ...
    for i in range(max_iter):
        ...
        for th, fac in zip(
            jump_factor_settings["thresholds"], jump_factor_settings["factors"]
        ):
            if np.abs(deltaz) < th:
                jump_factor = fac
                break
...
def get_multid_settings(studio):
    ...
    channels = [
        acqsttgs.channels().get(i) for i in range(acqsttgs.channels().size())
    ]

    z_definition = {
        "slices": [
            acqsttgs.slices().get(i) for i in range(acqsttgs.slices().size())
        ],
...
def start_acq(core, acq_settings):
    ...
    acquisition_config["outer dimensions"]["C"] = [
        str(c) for c in acquisition_config["outer dimensions"]["C"]
    ]
    acquisition_config["outer dimensions"]["S"] = [
        str(c) for c in acquisition_config["outer dimensions"]["S"]
    ]
    acquisition_config["outer dimensions"]["Z"] = [
        float(c) for c in acquisition_config["outer dimensions"]["Z"]
    ]
    print(acquisition_config)
    with open(
        os.path.join(dirpath, "acquisition_configuration.yaml"), "w"
    ) as f:
        ...
        for i_c, c in zp_iterate_C(zpacq, core):
            for i_z, z, (zdrive_pos, pfsoffset_pos) in zp_iterate_Z(
                ...
                with Acquisition(
                    directory=dirpath,
                    name=prefix,
                    show_display=acq_settings["show_display"],
                ) as acq:
                    acq.acquire(events)
                ...
                if (
                    viewer is not None
                    and acq_settings["close_display_after_acquisition"]
                ):
                    viewer.close()
    ...
    # write log
    with open(os.path.join(dirpath, "acquisition_log.yaml"), "w") as f:
        yaml.dump(acq_log, f)
    ...
    if not acq["usedims"]["Z"] or len(acq["Z"]) == 0:
        ...
        for i, zpos in enumerate(acq["Z"]):
            ...
            zdrive_pos, pfsoffset_pos = move_pfsoffset(
                ...
                core,
                acq_settings["ztags"],
                acq_settings["jump_factor_settings"],
                acq_settings["zmaxiter"],
                acq_settings["ztolerance"],
            )
...
if __name__ == "__main__":
    ...
    acq_settings = {
        ...
        "ztolerance": 0.025,  # tolerance of z movement in um
